...
struct h_ca_system_t {
  h_ca_systemey_t *systemey;
...
static h_core_bool_t create_new_current_state(h_ca_system_t *system);

static h_ca_state_t *get_current_state(h_ca_system_t *system);
    ...
    unsigned long initial_time_step_count, h_ca_systemey_t *systemey)
{
  ...
  if (system) {
    ...
    system->state_history = h_container_array_create
      (initial_state_history_array_size, h_ca_state_compare,
    ...
    if (system->state_history) {
      ...
      while ((state = h_container_array_iterate_next(initial_state_history))) {
        ...
        state_copy = h_ca_state_copy(state);
        h_container_array_add(system->state_history, time_step, state_copy);
        time_step++;

        state_cell_count = h_ca_state_get_cell_count(state);
...
{
  return h_ca_state_get_cell_count(get_current_state(system));
}
    ...
    unsigned long time_step, unsigned long cell_index)
{
  ...
  cell = h_ca_state_get_cell(state, cell_index);
...
{
  return h_ca_system_get_cell(system,
...
void h_ca_system_run(h_ca_system_t *system, h_core_run_t *run)
{
  ...
  while (!h_core_run_is_complete(run)) {
    ...
    cell_count = h_ca_state_get_cell_count(current_state);
    ...
    if (create_new_current_state(system)) {
      ...
      for (cell_index = 0; cell_index < cell_count; cell_index++) {
        new_cell_state = system->systemey->calculate_new_cell_state
          (system, cell_index);
        h_ca_system_set_cell(system, system->current_time_step + 1,
  ...
  if (image_buffer) {
    ...
    while ((state = h_container_array_iterate_next(system->state_history))) {
      for (each_cell = 0; each_cell < cell_count; each_cell++) {
        cell = h_ca_state_get_cell(state, each_cell);
    ...
    char *filename)
{
  ...
  if (file) {
    h_container_array_iterate_start(system->state_history);
    while ((state = h_container_array_iterate_next(system->state_history))) {
      cell_count = h_ca_state_get_cell_count(state);
      for (each_cell = 0; each_cell < cell_count; each_cell++) {
        cell_value = h_ca_state_get_cell_value(state, each_cell);
    ...
    unsigned long cell_index, h_ca_t *cell)
{
  ...
  state = h_container_array_find(system->state_history, time_step);
  h_ca_state_set_cell(state, cell_index, cell);
}
...
void h_ca_system_set_random_seed(h_ca_system_t *system, unsigned long seed)
{
  assert(system);
...
{
  h_ca_system_set_cell(system,
      system->current_time_step + relative_time_step, cell_index, cell);
}

h_core_bool_t create_new_current_state(h_ca_system_t *system)
{
  assert(system);
  h_core_bool_t success;
  h_ca_state_t *old_current_state;
  h_ca_state_t *new_current_state;

  old_current_state = get_current_state(system);
  new_current_state = h_ca_state_copy(old_current_state);
  if (new_current_state) {
    ...
    success = h_core_bool_false;
    h_core_trace("h_ca_state_copy");
  }

  return success;
}

h_ca_state_t *get_current_state(h_ca_system_t *system)
{
